...
| **Sluggish / missed events** | enable the kernel task log — flip `LOG_AK_KERNEL_ENABLE` to `-DAK_TASK_LOG_CONSOLE_ENABLE` in `ak.cfg.mk`, rebuild, capture (recipe: [kernel-task-log](ak://guide/kernel-task-log)) | `exeTime` > ~50 ms breaks run-to-completion — split the handler; big `waitTime` → raise the task's priority or shorten higher-priority handlers. **Turn it back off after** — the log itself costs ~6 ms UART per message |
