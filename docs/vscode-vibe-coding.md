...
2. Bấm icon 🔧 (**Tools**) → thấy nhóm `ak-docs` với 6 tool: `start_ak_project`, `search_ak_docs`, `get_ak_api`, `list_ak_api`, `get_ak_guide`, `get_ak_guardrails` → tick bật.
...
- **Tools:** `start_ak_project`, `search_ak_docs`, `get_ak_api`, `list_ak_api`, `get_ak_guide`, `get_ak_guardrails`
- **Guides:** start-project, create-task, create-driver, create-screen, use-timer, isr-bridge, tune-pools
- **Prompts:** `ak-new-project`, `ak-new-task`, `ak-new-driver` (template scaffolding kèm guardrails)
